...
module Server (M: Common.M_t) => {
  ...
  external makeOptions :
    ...
    createOptionsT =
    "" [@@bs.obj];
  external createWithOptions : createOptionsT => serverT =
    "socket.io" [@@bs.module];
  external createWithHttpAndOption : 'a => createOptionsT => serverT =
    "socket.io" [@@bs.module];
  external createWithPort : int => createOptionsT => serverT =
    "socket.io" [@@bs.module];

  /** */
  external serveClient : serverT => Js.boolean => serverT =
    "serveClient" [@@bs.send];
  ...
  external attach : serverT => 'a => createOptionsT => serverT =
    "attach" [@@bs.send];
  ...
  module Socket = {
    ...
    /* Here 'a means that you can send anything you want, and it'll depend on
       Bucklescript */
    external _on :
      socketT => (M.t 'a) [@bs.ignore] => string => ('a => unit) => unit =
      "on" [@@bs.send];
    ...
    type broadcastT;
    external _unsafeGetBroadcast : socketT => broadcastT =
      "broadcast" [@@bs.get];
    let broadcast socket str data =>
      _emit
        (_unsafeGetBroadcast socket) (M.stringify str) (Json.toValidJson data);
    external join : socketT => string => ('a => unit) => socketT =
      "join" [@@bs.send];
    external leave : socketT => string => ('a => unit) => socketT =
      "leave" [@@bs.send];
    external to_ : socketT => string => socketT = "to" [@@bs.send];
    external selectRoom : socketT => string => socketT = "to" [@@bs.send];
    external compress : socketT => Js.boolean => socketT =
      "compress" [@@bs.send];
    external disconnect : socketT => Js.boolean => socketT =
      "disconnect" [@@bs.send];
    external use : socketT => ('a => (unit => unit) => unit) => unit =
      "use" [@@bs.send];
    ...
    let volatileEmit (server: socketT) (event: M.t 'a) (obj: 'a) :unit =>
      _volatileEmit
        (getVolatile server) (M.stringify event) (Json.toValidJson obj);
  };
